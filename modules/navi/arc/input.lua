local _navi_path = (...):match("(.-)[^%.]+$")
local _,_,_,tab,_ = unpack(require(_navi_path .. 'code'))
local draw = require(_navi_path .. 'draw')
local sound = require(_navi_path .. 'sound')
...
function _key:check(dt)
    local z = self.pos
    self.t = self.t+dt
    if (z==0 and _navi.arc.btn.kp==self.btn) or (z==1 and lk.isDown(self.btn) and self.t>_navi.arc.cfg.key_wait0) then
        self.pos = self.pos+1
        self.t = 0
        self.hit = true
        return
    end
    if z == 2 and lk.isDown(self.btn) and self.t>_navi.arc.cfg.key_wait then
        self.t = self.t-_navi.arc.cfg.key_wait
        self.hit = true
        return
    end
...
function _inp_area:run()
    ...
    if self.ony then
        if _navi.arc.key.u.hit then self.iy = self.iy-1 end
        if _navi.arc.key.d.hit then self.iy = self.iy+1 end
    end
    if self.onx then
        if _navi.arc.key.r.hit then self.ix = self.ix+1 end
        if _navi.arc.key.l.hit then self.ix = self.ix-1 end
    end
    if (ix~=self.ix or iy~=self.iy) then
        sound.play(_navi.arc.sfx.cursor)
    end
...
function _choice:new(l,nvy,xbar)
    ...
    local ys = tab.range(0,_navi.arc.fn.h*(#l-1),_navi.arc.fn.h)
...
function _choice:run(x,y)
    ...
    for i = self.ia.ivy, self.ia.ivy+self.ia.nvy-1 do
        draw.text(self.l[i],x+_navi.arc.img.cursor:getWidth()+_navi.arc.cfg.cur_dx,y+_navi.arc.fn.h*(i-self.ia.ivy)+_navi.arc.cfg.cur_dy)
    end
    if self.xbar and (self.ia.ny > self.ia.nvy) then
        hbar = _navi.arc.fn.h*(self.ia.nvy)-2
    ...
    if self.ia.btn == 'ent' then
        sound.play(_navi.arc.sfx.confirm)
        self.done = true
