...
function _bloc:draw(x,y,nc)
    ...
    _navi.draw.text(self.s:sub(1,nc),x+self.x,y+self.y,self.c)
end

function _navi:setFont(font)
    _navi.arc.fn.f = font
    _navi.arc.fn.h = _navi.arc.fn.f:getHeight()+2
    _navi.arc.fn.w = function(s) return _navi.arc.fn.f:getWidth(s) end
    lg.setFont(_navi.arc.fn.f)
end
function _navi:new(s,opt)
    ...
    -- a ton of options!
    local opt = opt or {}
    if _navi.template then
        for k,v in pairs(_navi.template) do
            opt[k] = opt[k] or _navi.template[k]
        end
    end
    local w = opt.w                                     -- text wrap width
    o.font = opt.font or oldFont
    ...
    o.msg_spd = opt.msg_spd or _navi.arc.cfg.msg_spd    -- message speed
    ...
    o.rchs = opt.rchoices
    -- if o.rchs then
    --     for i,v in ipairs(o.chs) do
    --         o.rchs[i] = o.rchs[i] or nil
    --     end
    -- end

    _navi:setFont(o.font)


    o.dytext = (s=='' and -_navi.arc.fn.h) or 0 -- if message is '', move choices up one line
    o.dyname = (o.name and _navi.arc.fn.h) or 0
    -- set face info
    if o.face and not type(self.face_pos) == 'table' then
    ...
    local lwchs
    if o.chs then
        lwchs,wcur = {},_navi.arc.img.cursor:getWidth()+3
        for i = 1,o.nchs do lwchs[i] = _navi.arc.fn.w(o.chs[i])+wcur end
    ...
    local ps = string.rep(' ',_navi.arc.cfg.msg_nc_short_pause)
    local pl = string.rep(' ',_navi.arc.cfg.msg_nc_long_pause)
    s = s:gsub('|,',ps..'|,')
    s = s:gsub('|:',pl..'|:')
    ...
    local cc = _navi.arc.col.white    -- current text color
    ...
    while true do -- parse all the information out!
        ...
        if z == '|!' then
            b = s:sub(j,i-1)
            dx,dy = _navi.arc.fn.w(str.unformat(b)),0
        elseif z == '|c' then
            b = s:sub(j,i-1)
            dx,dy = _navi.arc.fn.w(str.unformat(b)),0
        elseif z == '|n' then
            b = s:sub(j,i-1)
            dx,dy = -xc,_navi.arc.fn.h
        elseif z == '|,' then
            b = s:sub(j,i-1)
            dx,dy = -_navi.arc.fn.w(ps)+_navi.arc.fn.w(str.unformat(b)),0
        elseif z == '|:' then
            b = s:sub(j,i-1)
            dx,dy = -_navi.arc.fn.w(pl)+_navi.arc.fn.w(str.unformat(b)),0
        end
        ...
        if b ~= '' then
            ...
            -- update line #
            if (z == '|n') then
                o.lw[cl] = xc+_navi.arc.fn.w(str.unformat(b))
                o.lb[cl] = e
...
function _navi:set_pos()
    ...
    self.hbox = self.hls+_navi.arc.fn.h+self.dyname+6
    ...
    self.yname = y+10
    if self.face and not type(self.face_pos) == 'table' then
        self.xface = x+10
    ...
    self.ychc = y+10+self.dyname+_navi.arc.fn.h*self.nl+self.dytext+2
    -- self.xblink = x+math.floor(0.5*(self.wbox-_navi.arc.img.blinker:getWidth()))
    self.xblink = x+math.floor(self.wbox-_navi.arc.img.blinker:getWidth())
    -- self.yblink = y+self.hbox-math.floor(0.5*_navi.arc.img.blinker:getHeight())
    self.yblink = y+self.hbox-math.floor(0.85*_navi.arc.img.blinker:getHeight())
...
function _navi:play(x,y)
    ...
    -- reset x,y
    if self.face and type(self.face_pos) == 'table' then
        self.xface = -x
        self.yface = -y
    end
    ...
    if self.box and (animo or animc) then
        af = (lt.getTime()-self.t0)/_navi.arc.cfg.msg_tanim
        _navi.draw.win_anim(self.xbox, self.ybox, self.wbox, self.hbox, af, self.pos)
        if af > 1 then
    ...
    -- draw stuff
    local drawFaces = function()
        lg.setColor(_navi.arc.col.white)
        if self.face and type(self.face) == 'table' then
            for i,v in ipairs(self.face) do
                lg.draw(self.face[i], self.xface, self.yface)
            end
    ...
    if self.face and type(self.face_pos) == 'table' and self.face_pos.z then
        drawFaces()
    end
    if self.box then _navi.draw.window(self.xbox, self.ybox, self.wbox, self.hbox) end
    if self.name then _navi.draw.text(self.name, self.xname, self.yname, _navi.arc.col.name) end
    if self.face and (not type(self.face_pos) == 'table' or not self.face_pos.z)then
        drawFaces()
    end
    ...
    if self.pos == 'scroll' and self.scroll then
        ...
        dys = -(self.view-1)*_navi.arc.fn.h
        af = (lt.getTime()-self.sct0)/_navi.arc.cfg.msg_tscroll
        if af >= 1 then
            self.pos = self.scpos
            ...
            for i = self.scbf,self.scbl do
                self.b[i]:draw(self.xtext+self.bdx[i], self.ytext+dys-af*_navi.arc.fn.h*self.nrows)
            end
        end
        lg.setScissor()
        lg.pop()
    ...
    if self.pos == 'choice' then
        if self.chs then
            if self:chc_fits() then --  then -- start new screen
                self.dychs = (-self.nl+self.nl%self.nrows)*_navi.arc.fn.h
            else
                self.view = self.view+self.nrows
                self.dychs = -self.nl*_navi.arc.fn.h
            end
        end
        self.pos = 'choose'
    end
        ...
        for i = bf,bl do
            ...
            if self.pos == 'show' and nc <= self.rnc[i] then -- display the current bloc
                ...
                if self.b[i].inp and not self.b[i].done then
                    ...
                    if self.b[i].tset then
                        self:show_blinker()
                        if _navi.arc.btn.kp == _navi.arc.btn.ent then -- if player pressed input key
                            self.b[i].done = true
    ...
    if self.pos == 'show' and _navi.arc.btn.kp == _navi.arc.btn.ent and self.skip and (not self.b[self.ci].inp or (self.b[self.ci].inp and (not self.b[self.ci].tset or self.b[self.ci].done))) then
        for i = bf,bl-1 do
            if self.b[i].inp then
                self.b[i].done = true
            end
        end
        self.t0 = lt.getTime()-self.rnc[self.lb[math.min(self.view+self.nrows-1,self.nl)]]/_navi.arc.cfg.msg_spd
    end
    ...
    if self.pos == 'choose' then
        if self.chs then
            if not self.cchs.done then
                ...
                if self.rchs then
                    self.pick[2] = self.rchs[self.cchs.pick]
                end
            end
        else
            if self.wait == nil then
                self:show_blinker()
                if _navi.arc.btn.kp == _navi.arc.btn.ent then
                    self.pos = 'close'
                    self.t0 = lt.getTime()
                end
...
function _navi:show_blinker()
    if ((lt.getTime()-self.t0)%_navi.arc.cfg.msg_tblink < 0.5*_navi.arc.cfg.msg_tblink) and self.blinker then
        lg.setColor(_navi.arc.col.white)
        lg.draw(_navi.arc.img.blinker, self.xblink, self.yblink)
    end
end
...
function _navi.add_nls(s,w,opt)
    ...
    for i = 1,s:len() do
        if s:sub(i,i) == ' ' or s:sub(i,i+1) == '|n' then
            if si and sj and _navi.arc.fn.w(b:sub(bj,j-1)) > w then -- there must be at least one ' ' before this
